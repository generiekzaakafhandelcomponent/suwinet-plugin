...
export class RdwVoertuigenComponent
    implements FunctionConfigurationComponent, OnInit, OnDestroy {
    @Input() save$: Observable<void>;
    @Input() disabled$: Observable<boolean>;
    @Input() pluginId: string;
    @Input() prefillConfiguration$: Observable<RdwVoertuigenConfig>;
    @Output() valid: EventEmitter<boolean> = new EventEmitter<boolean>();
    @Output() configuration: EventEmitter<RdwVoertuigenConfig> = new EventEmitter<RdwVoertuigenConfig>();

    private saveSubscription!: Subscription;
    private readonly formValue$ = new BehaviorSubject<RdwVoertuigenConfig | null>(null);
    ...
    private handleValid(formValue: RdwVoertuigenConfig): void {
        const valid = !!(
            formValue.bsn &&
