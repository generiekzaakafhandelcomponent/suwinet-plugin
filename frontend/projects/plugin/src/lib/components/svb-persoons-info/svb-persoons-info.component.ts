...
export class SvbPersoonsInfoComponent
    implements FunctionConfigurationComponent, OnInit, OnDestroy {
    @Input() save$: Observable<void>;
    @Input() disabled$: Observable<boolean>;
    @Input() pluginId: string;
    @Input() prefillConfiguration$: Observable<SVBPersoonsInfoComponent>;
    @Output() valid: EventEmitter<boolean> = new EventEmitter<boolean>();
    @Output() configuration: EventEmitter<SVBPersoonsInfoComponent> = new EventEmitter<SVBPersoonsInfoComponent>();

    private saveSubscription!: Subscription;
    private readonly formValue$ = new BehaviorSubject<SVBPersoonsInfoComponent | null>(null);
