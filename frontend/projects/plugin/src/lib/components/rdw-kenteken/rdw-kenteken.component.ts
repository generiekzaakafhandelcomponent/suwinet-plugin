...
export class RdwKentekenComponent
    implements FunctionConfigurationComponent, OnInit, OnDestroy {
    @Input() save$: Observable<void>;
    @Input() disabled$: Observable<boolean>;
    @Input() pluginId: string;
    @Input() prefillConfiguration$: Observable<RdwKentekensConfig>;
    @Output() valid: EventEmitter<boolean> = new EventEmitter<boolean>();
    @Output() configuration: EventEmitter<RdwKentekensConfig> = new EventEmitter<RdwKentekensConfig>();

    private saveSubscription!: Subscription;
    private readonly formValue$ = new BehaviorSubject<RdwKentekensConfig | null>(null);
    ...
    private handleValid(formValue: RdwKentekensConfig): void {
        const valid = !!(
            formValue.bsn &&
