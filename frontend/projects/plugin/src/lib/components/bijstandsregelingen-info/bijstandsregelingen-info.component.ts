...
export class BijstandsregelingenInfoComponent
    implements FunctionConfigurationComponent, OnInit, OnDestroy {
    @Input() save$: Observable<void>;
    @Input() disabled$: Observable<boolean>;
    @Input() pluginId: string;
    @Input() prefillConfiguration$: Observable<BijstandsRegelingenInfoConfig>;
    @Output() valid: EventEmitter<boolean> = new EventEmitter<boolean>();
    @Output() configuration: EventEmitter<BijstandsRegelingenInfoConfig> = new EventEmitter<BijstandsRegelingenInfoConfig>();
    ...
    private handleValid(formValue: BijstandsRegelingenInfoConfig): void {
        const valid = !!(
            formValue.bsn &&
