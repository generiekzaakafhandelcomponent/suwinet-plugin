...
import com.ritense.valtimoplugins.dkd.Bijstandsregelingen.BijstandsregelingenInfoResponse.ClientSuwi.AanvraagUitkering.BeslissingOpAanvraagUitkering
...
class SuwinetBijstandsregelingenService(
        ...
        return when (responseValue.value) {
            is ClientSuwi -> {
                ...
                return BijstandsRegelingenDto(
                    ...
                    specifiekeGegevensBijzBijstandList =
                        getSpeciekeGegevensBijzBijstand(
                            bijstandsRegelingenInfo.specifiekeGegevensBijzBijstand,
                        ),
    ...
    private fun getVorderingen(vorderingen: MutableList<ClientSuwi.Vordering>): List<VorderingDto> =
        vorderingen.map { vordering ->
            VorderingDto(
                ...
                partnersVordering =
                    vordering.partnerVordering?.let { getPartners(vordering.partnerVordering) } ?: mutableListOf(),
                szWet = SzWetDto(cdSzWet = vordering.szWet.cdSzWet),
            )
        }

    private fun getPartners(
        partnerVordering: MutableList<ClientSuwi.Vordering.PartnerVordering>,
    ): MutableList<PartnerDto> =
    ...
    private fun getSpeciekeGegevensBijzBijstand(
        specifiekeGegevensBijzBijstand: MutableList<ClientSuwi.SpecifiekeGegevensBijzBijstand>,
    ): List<SpecifiekeGegevensBijzBijstandDto> =
        specifiekeGegevensBijzBijstand.map { specifiekeGegevensBijzBijstandItem ->
            SpecifiekeGegevensBijzBijstandDto(
                ...
                datBetaalbaarBijzBijstand =
                    dateTimeService.toLocalDate(
                        specifiekeGegevensBijzBijstandItem.datBetaalbaarBijzBijstand,
    ...
    private fun getAanvraagUitkeringen(
        aanvraagUitkering: MutableList<ClientSuwi.AanvraagUitkering>,
    ): List<AanvraagUitkeringDto> =
        aanvraagUitkering
            .map { aanvraag ->
                AanvraagUitkeringDto(
                    datAanvraagUitkering =
                        dateTimeService.toLocalDate(
                            aanvraag.datAanvraagUitkering,
                            SUWINET_DATEIN_PATTERN,
                        ),
                    szWet = SzWetDto(aanvraag.szWet?.cdSzWet),
                    beslissingOpAanvraagUitkering =
                        getBeslissingOpAanvraagUitkering(
                            aanvraag.beslissingOpAanvraagUitkering,
                        ),
    ...
    private fun getBron(bron: Bron?): BronDto =
        BronDto(
    ...
    private fun getBeslissingOpAanvraagUitkering(
        beslissingOpAanvraagUitkering: ClientSuwi.AanvraagUitkering.BeslissingOpAanvraagUitkering?,
        ...
        beslissingOpAanvraagUitkering?.let { beslissing ->
            BeslissingOpAanvraagUitkeringDto(
                ...
                datDagtekeningBeslisOpAanvrUitk =
                    dateTimeService.toLocalDate(
                        beslissing.datDagtekeningBeslisOpAanvrUitk,
                        SUWINET_DATEIN_PATTERN,
                    ),
            )
        }
